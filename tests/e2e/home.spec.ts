...
test('renders the home page and links to the reserved modules', async ({ page }) => {
  ...
  await expect(page.getByRole('img', { name: '佟金龙简历第一页' })).toHaveCount(0)
  ...
  await expect(page.getByLabel('3D 空间沉浸场景')).toBeVisible()
  await expect(page.getByRole('heading', { name: '空间世界' })).toHaveCount(0)
